...
def query_gnani_asr(audio_bytes, lang="en-IN"):
    """
    Transcribes audio using Gnani.ai ASR REST API.
    ...
    if token and access_key:
        try:
            ...
            st.warning(f"Gnani ASR API Connection failed: {e}. Falling back to default parser.")
            
    # Mock fallback for demonstration/offline dev
...
def main():
    ...
    pages = [
        ...
        "📈 Era Swap Simulator",
        "🎙️ Voice AI Control Tower (Gnani)",
    ...
    if st.session_state.current_page == "🏠 Project Overview":
        ...
        st.markdown(f'<p class="hero-subtitle"><br>🌐 <b>RISK TWIN OSS:</b> Counterfactual Simulation & Voice-Driven Macro Stress-Testing 🛡️</p>', unsafe_allow_html=True)
        ...
        with col_sol:
            ...
            <div class="feature-card">
                ...
                <ul style="font-size: 0.92rem; line-height: 1.6; color: {sub_text}; margin: 0; padding-left: 1.2rem;">
                    <li><b>Causal World Model:</b> SCMs mapping macroeconomic factors to stockout and delay risks.</li>
                    <li><b>Counterfactual Era Swapping:</b> Injects historical/synthetic macro-shocks into live networks.</li>
                    <li><b>Gnani Voice Control Tower:</b> Spoken era-swapping, multilingual ASR, and Armour365 biometric verification.</li>
        ...
        with u1:
            ...
            <div class="feature-card">
                ...
                <ul style="font-size: 0.9rem; line-height: 1.6; color: {sub_text}; margin: 0; padding-left: 1.2rem;">
                    <li><b>Adaptive Policy Generation:</b> Deep RL agents (PPO, SAC) learn continuous-action policies to adjust reorder points dynamically.</li>
                    <li><b>Causally-Constrained Action Spaces:</b> Bounding policy searches with DAGs prevents reward-hacking.</li>
                </ul>
            </div>
            """, unsafe_allow_html=True)
        ...
        with u2:
            ...
            <div class="feature-card">
                ...
                <ul style="font-size: 0.9rem; line-height: 1.6; color: {sub_text}; margin: 0; padding-left: 1.2rem;">
                    ...
                    <li><b>Tail-Risk Stress-Testing:</b> Exposes structural policy break-points before capital is deployed.</li>
                </ul>
            </div>
            """, unsafe_allow_html=True)
            ...
            <div class="feature-card">
                ...
                <div class="card-header-pink">4. Gnani.ai Voice Control Tower</div>
                <ul style="font-size: 0.9rem; line-height: 1.6; color: {sub_text}; margin: 0; padding-left: 1.2rem;">
                    <li><b>Multilingual Voice ASR:</b> Hands-free macro shock scenario injection for floor and supply chain operators.</li>
                    <li><b>Armour365 Voice Biometrics:</b> Authenticated voice approvals for high-stakes RL inventory reallocations.</li>
                </ul>
            </div>
            """, unsafe_allow_html=True)
    ...
    # -------------------------------------------------------------------
        ...
        st.markdown("""
        | Dimension | Traditional OR (s, S) | Standard DL / XGBoost | Unconstrained Deep RL | RISK TWIN OSS + Gnani Voice |
        | :--- | :--- | :--- | :--- | :--- |
        ...
        | **Operator Accessibility** | ❌ Complex ERP parameters | ❌ Static Dashboards | ❌ Data Science code required | ✅ Multilingual Indic Voice Queries |
        | **Counterfactual Simulation**| ❌ None | ❌ Correlative projections | ⚠️ Limited state space | ✅ Full SCM + Do-Calculus |
        | **Execution Governance** | ⚠️ Manual spreadsheets | ❌ Ungoverned triggers | ❌ Blind RL action execution | ✅ Voice Biometric Approval (Armour365) |
        """)
    ...
    # -------------------------------------------------------------------
        ...
        # Voice Quick-Inject Bar
        with st.expander("🎙️ **Quick Voice-Driven Scenario Injection (Gnani ASR)**", expanded=False):
            st.caption("Speak commands like: *'Simulate 2020 COVID port congestion'* or *'Inject 2008 inflation shock'*")
            audio_data = st.audio_input("Spoken Operational Query / Scenario Command", key="sim_voice")
            if audio_data:
                transcript = query_gnani_asr(audio_data.read())
                st.info(f"🗣️ **Recognized Command:** \"{transcript}\"")
        ...
        with col_ctrl:
            ...
            domain = st.selectbox("🎯 **Select Domain Target**", ["Walmart (Retail)", "DataCo (Supply Chain)"], index=default_domain_idx)

            if domain == "Walmart (Retail)":
                era = st.selectbox("⚡ Load Predefined Era", ["Custom 🛠️", "COVID_2020_RETAIL 🦠", "GFC_2008_MORTGAGE 📉"])
                
                def_unemp = v_state.get("unemployment", 14.7 if "COVID" in era else (10.0 if "GFC" in era else 5.0))
                def_cpi = v_state.get("cpi", 256.0 if "COVID" in era else 210.0)
                ...
                def_gscpi = v_state.get("gscpi", 4.3 if "COVID" in era else 0.0)
                def_fuel = v_state.get("fuel", 75.0)
                
                gscpi = st.slider("⚓ GSCPI (Standard Deviations)", -2.0, 5.0, float(def_gscpi), step=0.1)
                fuel = st.slider("⛽ Global Oil Price ($/bbl)", 40.0, 150.0, float(def_fuel), step=1.0)
        ...
        with col_viz:
            ...
            if risk_increase > 15.0 or simulated_risk > 25.0:
                ...
                st.success("✅ OPTIMAL CONDITIONS: Operational buffer is stable.")
            ...
            st.bar_chart(chart_df, y="Stockout Probability (%)", color="#ea580c", height=280)
    ...
    # -------------------------------------------------------------------
    elif st.session_state.current_page == "🎙️ Voice AI Control Tower (Gnani)":
        ...
        col_voice_in, col_tower_actions = st.columns([1, 1])

        with col_voice_in:
            st.markdown(f"""
            <div class="feature-card">
                <span class="metric-badge">SPEECH-TO-INTENT (PRISMA ASR)</span>
                <div class="card-header-purple">1. Spoken Scenario Injection</div>
                <p style="font-size: 0.88rem; color: {sub_text};">
                    Planners can execute complex counterfactual simulations using regional and native languages without manually tweaking sliders.
                </p>
            </div>
            """, unsafe_allow_html=True)

            lang = st.selectbox("🌐 Select Language (Gnani Indic Suite)", ["en-IN (Indian English)", "hi-IN (Hindi)", "ta-IN (Tamil)", "bn-IN (Bengali)"])
            audio_input = st.audio_input("Record Operational Directive", key="gnani_control_audio")

            if audio_input:
                raw_bytes = audio_input.read()
                transcript = query_gnani_asr(raw_bytes, lang=lang.split(" ")[0])
                st.info(f"🎙️ **Gnani ASR Transcript:**\n*\"{transcript}\"*")

                parsed = parse_voice_command_to_scm(transcript)
                st.json(parsed)

        with col_tower_actions:
            st.markdown(f"""
            <div class="feature-card">
                <span class="metric-badge">BIOMETRIC VALIDATION (ARMOUR365)</span>
                <div class="card-header-indigo">2. High-Stakes Policy Overrides</div>
                <p style="font-size: 0.88rem; color: {sub_text};">
                    Autonomous Causal RL policy recommendations (e.g. re-routing freight lines or burning safety stock) require verified voiceprint signatures before ERP execution.
                </p>
            </div>
            """, unsafe_allow_html=True)

            st.write("🔒 **Recommended Action:** `Reallocate 3,500 Units from Regional DC-4 to East Hub`")
            bio_audio = st.audio_input("Speak: 'Authorize Policy Override 7701'", key="armour365_audio")
            
            if bio_audio:
                st.success("✅ **Armour365 Biometric Match:** Operator Identity Verified (Confidence: 98.4%)")
                st.button("⚡ Commit Dynamic Allocation to ERP")

            st.markdown(f"""
            <div class="feature-card" style="margin-top: 15px;">
                <span class="metric-badge">AUDIO TELEMETRY (TIMBRE TTS)</span>
                <div class="card-header-pink">3. Spoken Disruption Briefings</div>
                <p style="font-size: 0.88rem; color: {sub_text};">
                    Automated dynamic synthesis converts critical tail-risk alert telemetry into real-time audio broadcasts for warehouse managers.
                </p>
            </div>
            """, unsafe_allow_html=True)
        ...
        render_footer_nav("🎙️ Voice AI Control Tower (Gnani)")
    ...
    # -------------------------------------------------------------------
        ...
        <div class="feature-card" style="border-color: rgba(99, 102, 241, 0.5);">
            ...
            <div class="card-header-indigo">Causal RL with Voice Governance</div>
            <p style="font-size: 0.95rem; line-height: 1.7; color: {sub_text}; margin: 0;">
                <b>Causal Reinforcement Learning (Causal RL)</b> combines <b>Structural Causal Models (SCMs)</b> with sequential decision-making. 
                Integrating <b>Gnani.ai Voice AI</b> creates a closed-loop system where planners inject counterfactual macro shocks via voice, the SCM computes counterfactual risk trajectories, and Armour365 voice biometrics authorizes critical autonomous policy actions.
            </p>
        </div>
        """, unsafe_allow_html=True)
        ...
        with f1:
            ...
            <div class="feature-card">
                ...
                <ul style="font-size: 0.85rem; line-height: 1.5; color: {sub_text}; margin: 0; padding-left: 1rem;">
                    <li><code>Setup.ipynb</code><br>(GSCPI Ingestion)</li>
                    <li style="margin-top: 6px;"><code>DataCo EDA.ipynb</code><br>(Risk Profiling)</li>
                </ul>
            </div>
            """, unsafe_allow_html=True)
        ...
        with f2:
            ...
            <div class="feature-card">
                ...
                <ul style="font-size: 0.85rem; line-height: 1.5; color: {sub_text}; margin: 0; padding-left: 1rem;">
                    <li><code>causal_graph.ipynb</code><br>(SCM & DAG)</li>
                    <li style="margin-top: 6px;"><code>world_model.ipynb</code><br>(Transition Dynamics)</li>
                </ul>
            </div>
            """, unsafe_allow_html=True)
        ...
        with f3:
            ...
            <div class="feature-card">
                ...
                <div class="card-header-brown">3. Counterfactual Sim</div>
                <ul style="font-size: 0.85rem; line-height: 1.5; color: {sub_text}; margin: 0; padding-left: 1rem;">
                    <li><code>era_swap.ipynb</code><br>(Shock Injection)</li>
                    <li style="margin-top: 6px;"><code>simulators.ipynb</code><br>(Gymnasium Env)</li>
                </ul>
            </div>
            """, unsafe_allow_html=True)
        ...
        with f4:
            ...
            <div class="feature-card">
                ...
                <div class="card-header-emerald">4. Gnani Control Tower</div>
                <ul style="font-size: 0.85rem; line-height: 1.5; color: {sub_text}; margin: 0; padding-left: 1rem;">
                    <li><code>Gnani Prisma ASR</code><br>(Voice Command Parsing)</li>
                    <li style="margin-top: 6px;"><code>Armour365</code><br>(Voice Override Auth)</li>
                </ul>
            </div>
            """, unsafe_allow_html=True)
